...
import { HeartPulse, Loader2, ServerCrash, PackageOpen, Phone, MapPin, Clock } from "lucide-react";
import { perawatApi, type BackendPerawat } from "@/lib/api";
...
function LoadingState() {
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4">
      ...
      <p className="text-sm text-neutral-500">Memuat data perawat…</p>
    </div>
  );
}
...
function ErrorState({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4 text-center">
      ...
      <div>
        <p className="font-semibold text-neutral-800">Gagal memuat data perawat</p>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center min-h-[40vh] gap-4 text-center">
      ...
      <div>
        <p className="font-semibold text-neutral-700">Belum ada data perawat</p>
        <p className="text-sm text-neutral-400 mt-1">
          Data perawat belum tersedia di database.
        </p>
      </div>
    </div>
  );
...
/* ─────────────────────────────────────────────
   Shift Badge Color Map
───────────────────────────────────────────── */
const SHIFT_COLORS: Record<string, { bg: string; text: string }> = {
  Pagi:  { bg: "var(--color-success-light)",  text: "var(--color-success)" },
  Siang: { bg: "var(--color-warning-light)",  text: "var(--color-warning)" },
  Malam: { bg: "#ede9fe",                      text: "#7c3aed" },
};

function getShiftColor(shift: string) {
  return SHIFT_COLORS[shift] ?? { bg: "var(--color-primary-50)", text: "var(--color-primary-700)" };
}

/* ─────────────────────────────────────────────
   Perawat Card
───────────────────────────────────────────── */
function PerawatCard({ perawat, index }: { perawat: BackendPerawat; index: number }) {
  const shiftColor = getShiftColor(perawat.shift);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: "var(--color-earth-500)" }}
      />
      ...
      <div className="mt-4 space-y-2">
        <div className="flex items-center gap-2 text-sm text-neutral-600">
          ...
          <span className="truncate">{perawat.ruangan}</span>
        </div>
        <div className="flex items-center gap-2 text-sm text-neutral-600">
          <Phone size={14} className="text-neutral-400 flex-shrink-0" />
          <span>{perawat.telp}</span>
        </div>
        <div className="flex items-center gap-2 text-sm text-neutral-600">
          <Clock size={14} className="text-neutral-400 flex-shrink-0" />
          <span className="text-xs text-neutral-500">Shift: {perawat.shift}</span>
        </div>
      </div>
    </motion.div>
  );
...
export default function PerawatPage() {
  const [perawatList, setPerawatList] = useState<BackendPerawat[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const fetchPerawat = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const data = await perawatApi.getAll();
      setPerawatList(data);
    } catch (err) {
      const msg =
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      >
        <div
          ...
          style={{
            background: "linear-gradient(135deg, var(--color-earth-500), var(--color-earth-400))",
            boxShadow: "0 4px 12px rgb(120 80 40 / 0.2)",
          }}
        >
          <HeartPulse size={21} className="text-white" />
        </div>
        <div>
          <h1
          ...
          <p className="text-sm text-neutral-500 mt-1">
            {loading
              ...
              : error
              ? "Gagal memuat data"
              : `${perawatList.length} perawat terdaftar di RSUD Ngawi`}
          </p>
        </div>
      </motion.div>
      ...
      {loading ? (
        ...
        <ErrorState message={error} onRetry={fetchPerawat} />
      ) : perawatList.length === 0 ? (
        <EmptyState />
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {perawatList.map((p, i) => (
            <PerawatCard key={p.id} perawat={p} index={i} />
          ))}
        </div>
      )}
    </div>
